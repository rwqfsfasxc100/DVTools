...
func _ready():
	...
	lc.get_node("system/property_editor").connect("changed",self,"_change_system_display")
...
func _change_system_display():
	if current_button and can_change_sys_display:
		yield(get_tree(),"idle_frame")
...
func set_properties_from_dict(dict:Dictionary):
	...
	lc.get_node("hardpoint_type/TagPopup").initialize_current_tags(dict.get("hardpoint_type",""))
...
func get_this_dict_for_saving() -> Dictionary:
	...
	var hardpoint_type = lc.get_node("hardpoint_type/TagPopup").get_selected_string()
